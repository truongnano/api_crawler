...
async def fetch_user_post_videos(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_user_post_videos(sec_user_id, max_cursor, count)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_user_like_videos(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_user_like_videos(sec_user_id, max_cursor, counts)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_user_collection_videos(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_user_collection_videos(cookie, max_cursor, counts)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_user_mix_videos(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_user_mix_videos(mix_id, max_cursor, counts)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_user_live_videos(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_user_live_videos(webcast_id)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_user_live_videos_by_room_id(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_user_live_videos_by_room_id(room_id)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_live_gift_ranking(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_live_gift_ranking(room_id, rank_type)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_live_room_product_result(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_live_room_product_result(cookie, room_id, author_id, limit)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def handler_user_profile(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.handler_user_profile(sec_user_id)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_video_comments(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_video_comments(aweme_id, cursor, count)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def fetch_video_comments_reply(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.fetch_video_comments_reply(item_id, comment_id, cursor, count)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_real_msToken(request: Request):
    ...
    """
    try:
        data = await DouyinWebScraper.gen_real_msToken()
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_ttwid(request: Request):
    ...
    """
    try:
        data = await DouyinWebScraper.gen_ttwid()
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_verify_fp(request: Request):
    ...
    """
    try:
        data = await DouyinWebScraper.gen_verify_fp()
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_s_v_web_id(request: Request):
    ...
    """
    try:
        data = await DouyinWebScraper.gen_s_v_web_id()
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_x_bogus(request: Request,
    ...
    """
    try:
        x_bogus = await DouyinWebScraper.get_x_bogus(url, user_agent)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=x_bogus)
    except Exception as e:
...
async def generate_a_bogus(request: Request,
    ...
    """
    try:
        a_bogus = await DouyinWebScraper.get_a_bogus(url, user_agent)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=a_bogus)
    except Exception as e:
...
async def get_sec_user_id(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.get_sec_user_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_all_sec_user_id(request: Request,
   ...
   "urls":[
    ...
    """
    try:
        data = await DouyinWebScraper.get_all_sec_user_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_aweme_id(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.get_aweme_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_all_aweme_id(request: Request,
   ...
   "urls":[
    ...
    """
    try:
        data = await DouyinWebScraper.get_all_aweme_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_webcast_id(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.get_webcast_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_all_webcast_id(request: Request,
    ...
    """
    try:
        data = await DouyinWebScraper.get_all_webcast_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
